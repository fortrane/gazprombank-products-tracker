...
class UtilityClass
{
    ...
    public function checkSessions($permissions, $database, $uuid = null) {
        ...
        if($permissions == "defaultAccess") {
            if(!empty($_SESSION['id'])) {
                exit($this->changeLocationViaHTML('0', './review'));
            }
        }
    ...
    public function fetchDataFromExternalServer($url, $apiKey = null) {
        $ch = curl_init();
    ...
    public function sendDataToExternalServer($url, $data, $apiKey = null) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
